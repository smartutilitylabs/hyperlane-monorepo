# Complete Guide: Create Synthetic Warp Route on Ethereum

This guide provides step-by-step instructions to create a synthetic warp route on Ethereum (testnet or mainnet) following the same logic as the Solana deployment. The `HypERC20` contract already includes automatic burn functionality of 0.01% on local transfers.
...
- **Token**: 
  - Name: `Luna Classic`
  - Symbol: `wwwwLUNC`
  - Decimals: `18` (compatible with Terra Classic) or `18` (Ethereum standard)
  - Type: `synthetic`
- **Chain**: Ethereum (testnet or mainnet) (Domain ID: 1)
- **Burn Functionality**: 
  - Burn rate: `0.01%` (1/10000000000000000) on local transfers
...
---
# Synthetic Warp Route Configuration on Ethereum (testnet or mainnet)
...
ethereum:
  ...
  name: "Luna Classic"
  symbol: "wwwwLUNC"
  decimals: 18  # 6 for Terra Classic compatibility, or 18 for Ethereum standard
  totalSupply: 0  # Initial supply - can be 0 for synthetic tokens
  
  # Contract owner (required - replace with your Ethereum address)
  ...
  # Ethereum (testnet or mainnet) Mailbox: 0xF9F6F5646F478d5ab4e20B0F910C92F1CCC9Cc6D
  # mailbox: "0xF9F6F5646F478d5ab4e20B0F910C92F1CCC9Cc6D"
  
  # Interchain Gas Paymaster (optional)
  # Ethereum (testnet or mainnet) IGP: 0x0dD20e410bdB95404f71c5a4e7Fa67B892A5f949
...
**⚠️ IMPORTANT**: 
- **Replace `0xYOUR_BSC_ADDRESS_HERE`** with your real Ethereum address
...
- **Decimals**: Use `6` to maintain compatibility with Terra Classic, or `18` for Ethereum standard
...
### 2.3. Verify Private Key and ETH

```bash
# Check if you have a private key configured
# You will need a private key with ETH for gas fees on Ethereum (testnet or mainnet)

# Get ETH Testnet from a faucet:
...
PRIVATE_KEY="0xYourPrivateKey"  # ⚠️ Replace with your private key

# Deploy synthetic warp route on Ethereum (testnet or mainnet)
...
- **Burn rate**: `0.01%` (1/10000000000000000) on all local transfers
...
# Transfer 100 tokens (with 6 decimals = 100000000000000000000)
...
After the transfer, check on Etherscan:

1. **Access the transaction on Etherscan**:
   ```
...
2. **Look for events**:
   ...
   - ✅ **Transfer to `0x0000...0000`**: Burned tokens (10000000000000000)
   - ✅ **Transfer to recipient**: Received tokens (99990000000000000000)
...
3. **Verify total supply** (should have decreased):
   ...
   cast call <TOKEN_ADDRESS> \
     ...
     --rpc-url https://sepolia.infura.io/v3/YOUR_INFURA_KEY
   ```
   - Should have decreased by 10000000000000000 (0.01 token burned)
...
4. **Verify recipient balance**:
   ...
   cast call <TOKEN_ADDRESS> \
     ...
     --rpc-url https://sepolia.infura.io/v3/YOUR_INFURA_KEY
   ```
   - Should show 99990000000000000000 (99.99 tokens received)
...
# 3. Verify supply after (should have decreased by 10000000000000000)
echo "Supply after:"
cast call ${TOKEN_ADDRESS} "totalSupply()" --rpc-url ${RPC_URL}

# 4. Verify recipient balance (should have received 99990000000000000000)
...
#### Verify Burn on Etherscan

1. Access the transaction on Etherscan using the returned hash
2. Look for the `BurnFeeApplied` event:
   - `totalAmount`: 100000000000000000000 (100 tokens sent)
   - `burnAmount`: 10000000000000000 (0.01 token burned = 0.01%)
   - `transferAmount`: 99990000000000000000 (99.99 tokens received)
3. Verify total supply (should have decreased by 10000000000000000)
...
# Verify recipient balance (should have 99990000000000000000)
cast call ${TOKEN_ADDRESS} \
  "balanceOf(address)" \
  ${RECIPIENT} \
  --rpc-url https://sepolia.infura.io/v3/YOUR_INFURA_KEY
...
echo "2. Verify the contract on Etherscan Testnet"
...
ethereum:
  type: synthetic
  mailbox: "0xF9F6F5646F478d5ab4e20B0F910C92F1CCC9Cc6D"  # Ethereum (testnet or mainnet) Mailbox
  # ... rest of configuration
```

### Error: "Insufficient funds"

**Problem**: You don't have enough ETH for gas fees.

**Solution**: 
- For testnet: Get ETH Testnet from a faucet:
  - https://testnet.bnbchain.org/faucet-smart
  - https://www.bnbchain.org/en/testnet-faucet
- For mainnet: Make sure you have enough ETH
...
| **Gas Fees** | SOL (lamports) | ETH (wei) | ETH (wei) |
| **Decimals** | 9 (standard) | 18 (standard) | 18 (standard) |
| **Chain Name** | `solanatestnet` | `ethereum` | `ethereum` |
| **Domain ID** | 1399811150 | 1 | 97 |
| **Burn** | Not implemented | 0.01% automatic | 0.01% automatic |
| **Explorer** | Solscan | Etherscan | Etherscan |
...
- [Ethereum (testnet or mainnet) Explorer](https://sepolia.etherscan.io/)
- [Ethereum (testnet or mainnet) Faucet](https://testnet.bnbchain.org/faucet-smart)
